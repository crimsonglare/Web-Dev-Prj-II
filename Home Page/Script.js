...
function editTodo(id) {
  ...
  todos = todos.filter((todo) => todo.id !== id);
  add_btn.innerHTML = `<ion-icon name="checkmark-outline"></ion-icon>`;
  saveToLocalStorage();
  add_btn.addEventListener("click", () => {
    add_btn.innerHTML = `<ion-icon name="checkmark-outline"></ion-icon>`;
...
function toggleStatus(id) {
  ...
  todo.completed = !todo.completed;
  saveToLocalStorage();
  highlightRow(id); // Call the highlightRow function to toggle the highlight for the specific row
}

// Function to handle the highlighting
function highlightRow(id) {
  let row = document.querySelector(`.todo-item[data-id="${id}"]`);
